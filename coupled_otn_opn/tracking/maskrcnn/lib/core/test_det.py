...
def im_detect_all(model, im, box_proposals=None, timers=None):
    ...
    if cfg.MODEL.KEYPOINTS_ON and boxes.shape[0] > 0:
        ...
        cls_keyps = None
    cls_boxes = np.array(cls_boxes[1])
    return cls_boxes
...
def im_detect_bbox(model, im, target_scale, target_max_size, boxes=None):
    ...
    #return_dict = model(inputs['data'][0], inputs['im_info'][0])
...
def box_results_with_nms_and_limit(scores, boxes):  # NOTE: support single-batch
    ...
    n_det, num_classes = scores.shape
    new_scores = np.zeros((n_det, 2), dtype=np.float32)
    new_scores[:, 0] = scores[:, 0]
    new_scores[:, 1] = 1 - new_scores[:, 0]

    scores, boxes = new_scores, new_boxes
